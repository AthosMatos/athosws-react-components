...
const SubSubOption = ({ subsubopt, id, isSelected }: MenuSubSsubOptionProps) => {
  ...
  const colors = useMemo(() => {
    if (!subsubOptsColors) {
      if (!subOptsColors) {
        return isSelected ? optColors?.clicked : optColors?.normal;
      }
      return isSelected ? subOptsColors?.clicked : subOptsColors?.normal;
    }
    return isSelected ? subsubOptsColors?.clicked : subsubOptsColors?.normal;
  ...
  return (
    <ColoredDiv onClick={selectedOpt} colors={colors} className="w-full rounded-md cursor-pointer">
      <BsFillGrid1X2Fill />
      {subsubopt.label}
    </ColoredDiv>
  );
